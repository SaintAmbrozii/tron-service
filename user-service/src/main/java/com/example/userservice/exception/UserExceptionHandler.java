...
public class UserExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ProblemDetail handleUserServiceException(UserServiceException ex, WebRequest request) {
        log.error("Intercepted UserServiceException. Message: {}. Status: {}", ex.getMessage(), ex.getStatus());
        return createProblemDetail(ex.getMessage(), ex.getStatus(), request);
    }

    @ExceptionHandler(Exception.class)
    public ProblemDetail handleOtherExceptions(Exception ex, WebRequest request) {
        log.error("Intercepted unexpected exception. Message: {}", ex.getMessage(), ex);
        return createProblemDetail("internal error", HttpStatus.INTERNAL_SERVER_ERROR, request);
    }

    @ExceptionHandler(ConstraintViolationException.class)
    public ProblemDetail handleConstraintViolationException(ConstraintViolationException ex, WebRequest webRequest) {
        log.error("Intercepted ConstraintViolationException. Message: {}", ex.getMessage());
        Map<String, String> errors = new HashMap<>();
        ex.getConstraintViolations().forEach(v -> {
            errors.put(v.getPropertyPath().toString(), "failed to pass validation");
        });
        ProblemDetail pd = createProblemDetail("Validation failure", HttpStatus.BAD_REQUEST, webRequest);
